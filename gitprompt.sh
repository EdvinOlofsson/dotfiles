...
function updatePrompt() {
  ...
  if [[ "${#git_status_fields[@]}" -gt 0 ]]; then
    ...
    __chk_gitvar_status() {
      ...
      if eval "[[ ${v} ]]" ; then
        if [[ "${3-}" != '-' ]] && [[ "${__GIT_PROMPT_SHOW_CHANGED_FILES_COUNT}" == "1" || "${1-}" == "REMOTE" ]]; then
          __add_status "\${GIT_PROMPT_${1}_START}\${GIT_${1}}\${GIT_PROMPT_${1}_END}\${ResetColor}"
        else
    ...
    local path=$(pwd)
    local skull=''
    local TRUNK_VERSION=7.3

    path=${path##*/dev/git/}
    path=${path%%/*}

    if command -v confd &> /dev/null; then
      if [ -n ${path} ]
      then
          # If you are in the tailf or webui-prime repo, make sure the confd version is set to the latest vsn version
          if [ "${path}" == "tailf" ] || [ "${path}" == "webui-prime" ]; then
              if [ $(confd --version | awk '{print substr($0,0,3)}') != $TRUNK_VERSION ]; then
                  skull=" ${Red}☠ "
              fi

          # Check if path contains the confd major version
          elif [[ "$path" != *"$(confd --version | awk '{print substr($0,0,3)}')"* ]]; then
              skull=" ${Red}☠ "
          fi

          # If you are in the nfvo3 repo, just output the sorced version
          if [ "${path}" == "nfvo3" ]
          then
              skull='|${Blue}$(ncs --version)${ResetColor}|'
          fi
      fi
    fi
